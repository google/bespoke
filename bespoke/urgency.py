...
class Rating(pydantic.BaseModel):
  ...
  def __str__(self) -> str:
    iso_time = datetime.datetime.fromtimestamp(self.time).isoformat()
    ...
    if rating.mode == mode:
      ...
      if rating.score in [1, 2]:
        ...
        if first_good is None:
          ...
          good_interval_length = max(good_interval_length, rating.time - first_good)
...
def compute_urgency(
  ...
  last_non_zero = next(r for r in reversed(history) if r.mode == mode and r.score != 0)
