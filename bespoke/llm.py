...
async def tag_sentence(
    ...
    hint_prompt = (
        ...
        "Use these words if appropriate to find the correct `heteronym_index` and `def_index`."
    )
    ...
    prompt = (
        ...
        "For each word, identify the correct `heteronym_index` and `def_index` that matches its meaning in the context of the sentence. "
    ...
    if config["provider"] == "openrouter":
        ...
        temperature = 0.0  # Use 0.0 for deterministic precise tagging
    ...
    return [
        (tag.occurance, (tag.dictionary_entry, tag.heteronym_index, tag.def_index)) 
