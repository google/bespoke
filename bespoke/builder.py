...
class UnitPool:
    ...
    def __init__(
        self, target_language: Language, card_index: CardIndex, difficulty: Difficulty
    ) -> None:
...
class DeckBuilder:
    ...
    def __init__(
        ...
        self._full_vocabulary = [
            word
            for words in VOCABULARY[target_language.code_name].values()
            for word in words
        ]
        ...
        async with asyncio.TaskGroup() as tg:
            while not sentence_producer.done():
                ...
                for sentence in sentences:
                    ...
                    tg.create_task(
                        self._complete_card(
                            semaphore, sentence, units_to_learn, grammar_to_learn
                        )
                    )
                    ...
                    if verbose:
                        for unit in units_to_learn:
                            if all(unit not in card.units for card in cards):
                                print(
                                    f"The unit '{unit}' was requested, but no generated."
                                )
                                for card in cards:
                                    print(card)
                    self._card_index.save()
