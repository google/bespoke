...
class RatingWebApp:
  ...
  def _render_sentence(self, text: str, large: bool = True) -> None:
    style = (
      "font-size: 30px; padding: 20px;" if large else "font-size: 18px; padding: 10px;"
    )
    html_content = f"""
      ...
      match self._mode:
        case Mode.LISTEN:
          with ui.card().classes("w-full items-center bg-gray-100"):
            self._render_audio_player("Play:", self._card.audio_filename, autoplay=True)
            self._render_audio_player("Slow:", self._card.slow_audio_filename)
        case Mode.SPEAK:
          ui.label("Speak the sentence!").classes("text-gray-500 text-xl font-mono")
          with ui.card().classes("w-full items-center bg-gray-100"):
        ...
        case Mode.WRITE:
          ui.label("Write the sentence!").classes("text-gray-500 text-xl font-mono")
          with ui.card().classes("w-full items-center bg-gray-100"):
            self._render_audio_player("", self._card.native_audio_filename)
          self._render_sentence(self._card.native_sentence)
  ...
  def _show_back(self) -> None:
    ...
    with self.main_container:
      ...
      with ui.card().classes("w-full items-center bg-gray-100"):
        ...
        self._render_audio_player("Play:", self._card.audio_filename, autoplay=autoplay)
        self._render_audio_player("Slow:", self._card.slow_audio_filename)
      ...
      ui.button("Next", on_click=lambda: self._finalize(report_switch.value)).props(
...
def open_deck() -> tuple[Deck, str]:
  ...
  parser.add_argument("--use_write_mode", action="store_true", help="Enable write mode")
